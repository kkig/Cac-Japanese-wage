...
const hoursTotalOne = () => {
  ...
  const lunchStart = lunchS.value;
  const lunchEnd = lunchE.value;
...
const hoursTotalTwo = () => {
  const startTimeTwo = startTwo.value;
  const endTimeTwo = endTwo.value;
  const lunchSrtTwoInput = lunchSrtTwo.value;
  const lunchEndTwoInput = lunchEndTwo.value;

  const hoursTwo = getWorkLength(startTimeTwo, endTimeTwo, lunchSrtTwoInput, lunchEndTwoInput);

  return hoursTwo.total;
}
...
function getWage() {
  ...
  const totalTime = hoursTotalOne() + hoursTotalTwo();
